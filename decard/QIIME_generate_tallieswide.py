...
def main():
    ...
    args_parser.add_argument('tallieswide')
    ...
    args_parser.add_argument('--suffix','-S',help='Suffix to add to specimen / label to keep track of condition')
    ...
    try: # Try first load things in with the janky biom-format.Table class.
        ...
        otu_matrix['otu_index'] = otu_matrix.raw.apply(lambda sm: sm.nonzero()[1][0])
        otu_matrix['weight'] = otu_matrix.apply(lambda r: r.raw[0,r.otu_index], axis=1)
    ...
    tallies_wide = pd.DataFrame()
    for community, c_block in otu_matrix.groupby('community'):
        comm_wide = pd.DataFrame()
        comm_wide[community]=c_block.groupby('name').sum().weight
        tallies_wide = pd.merge(tallies_wide,comm_wide,how='outer',left_index=True, right_index=True)
    ...
    tallies_wide_w.writerow(['tax_name','tax_d','rank']+[n+suffix for n in tallies_wide.columns])
    ...
    for name, row in tallies_wide.iterrows():
        otu = otu_df[otu_df.name==name].iloc[0]
        tax_id = otu.ncbi_tax_id
        rank = otu.ncbi_rank
        tallies_wide_w.writerow([name,tax_id,rank]+list(row))
